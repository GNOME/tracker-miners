...
tracker_extract_get_metadata (TrackerExtractInfo *info)
{
	...
	if (video_stream) {
		...
		if ((tag = find_tag (format, audio_stream, "track", NULL, 0))) {
		...
		if ((tag = find_tag (format, audio_stream, "album", NULL, 0))) {
			album_title = tag->value;
		}

		if (album_title && (tag = find_tag (format, audio_stream, "album_artist", NULL, 0))) {
			album_artist_name = tag->value;
			album_artist = tracker_extract_new_artist (album_artist_name);
		}

		if ((tag = find_tag (format, audio_stream, "artist", tag, 0))) {
			performer = tracker_extract_new_artist (tag->value);
		}

		if (!performer && (tag = find_tag (format, audio_stream, "performer", tag, 0))) {
			performer = tracker_extract_new_artist (tag->value);
		}
		...
		if ((tag = find_tag (format, audio_stream, "composer", tag, 0))) {
		...
		if (album_title) {
			...
			if ((tag = find_tag (format, audio_stream, "disc", NULL, 0))) {
