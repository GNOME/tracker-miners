...
tracker_extract_get_metadata (TrackerExtractInfo *info)
{
	...
	if (md.creator_name) {
		...
		if (vd.mb_artist_id) {
			TrackerResource *mb_artist_id = tracker_extract_new_external_resource("https://musicbrainz.org/doc/Artist",
											      vd.mb_artist_id);

			tracker_resource_set_relation (md.creator, "nfo:hasExternalReference", mb_artist_id);
	...
	if (vd.album) {
		...
		if (vd.mb_release_id) {
			mb_release_id = tracker_extract_new_external_resource("https://musicbrainz.org/doc/Release",
									      vd.mb_release_id);

			tracker_resource_set_relation (album, "nfo:hasExternalReference", mb_release_id);
			g_free (vd.mb_release_id);
		}

		if (vd.mb_release_group_id) {
			mb_release_group_id = tracker_extract_new_external_resource("https://musicbrainz.org/doc/Release_Group",
										    vd.mb_release_group_id);

			if (mb_release_id) {
				tracker_resource_add_relation (album, "nfo:hasExternalReference", mb_release_group_id);

			} else
				tracker_resource_set_relation (album, "nfo:hasExternalReference", mb_release_group_id);
	...
	if (vd.mb_recording_id) {
		mb_recording_id = tracker_extract_new_external_resource("https://musicbrainz.org/doc/Recording",
									vd.mb_recording_id);

		tracker_resource_set_relation (metadata, "nfo:hasExternalReference", mb_recording_id);
		g_free (vd.mb_recording_id);
	}

	if (vd.mb_track_id) {
		mb_track_id = tracker_extract_new_external_resource("https://musicbrainz.org/doc/Track",
								    vd.mb_track_id);

		if (mb_recording_id) {
			tracker_resource_add_relation (metadata, "nfo:hasExternalReference", mb_track_id);
			g_object_unref (mb_recording_id);
		} else {
			tracker_resource_set_relation (metadata, "nfo:hasExternalReference", mb_track_id);
		}
