...
static id3mbtype
id3_get_mb_type (const gchar *name)
{
	gint i;
	for (i = 0; i < G_N_ELEMENTS (id3_mbtypes); i++) {
		if ( strcmp (id3_mbtypes[i].name, name) == 0) {
			return id3_mbtypes[i].mbtype;
		}
	}
	return MB_UNKNOWN;
}
...
extract_txxx_tags (id3v2tag *tag, const gchar *data, guint pos, size_t csize, id3tag *info, gfloat version)
{
	...
	if (!tracker_is_empty_string (description)) {
		...
		mbtype = id3_get_mb_type (description);
	} else {
...
extract_ufid_tags (id3v2tag *tag, const gchar *data, guint pos, size_t csize)
{
	...
	if (tracker_is_empty_string (owner) ||
		(mb_type = id3_get_mb_type (owner)) == MB_UNKNOWN) {
		return;
	}
