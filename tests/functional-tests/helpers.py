...
class await_files_processed():
    ...
    def __enter__(self):
        if len(self.expected) == 1:
            log.info("Awaiting file-processed signal from %s for file %s",
                     self.miner, self.expected[0].path)
        else:
            log.info("Awaiting %i file-processed signals from %s",
                     len(self.expected), self.miner)

        def signal_cb(proxy, sender_name, signal_name, parameters):
            if signal_name == 'FileProcessed':
                uri, success, message = parameters.unpack()
        ...
        def timeout_cb():
            ...
            raise trackertestutils.helpers.AwaitTimeoutException(
                f"Timeout awaiting file-processed signal on {self.miner}")
...
class ExtractorHelper ():
    ...
    EXTRACTOR_BUSNAME = "org.freedesktop.Tracker1.Miner.Extract"
    EXTRACTOR_OBJ_PATH = "/org/freedesktop/Tracker1/Miner/Extract"
    MINER_IFACE = "org.freedesktop.Tracker1.Miner"
